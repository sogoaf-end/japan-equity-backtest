...
import { createCostModel, runBacktest } from "../backtest/engine.js";
...
async function main() {
  ...
  if (scope === "backtest" && command === "sma") {
    ...
    const lotSize = parseInteger(options["lot-size"], config.backtest.lotSize);
    const fixedCommission = parseNumber(options.commission, config.backtest.fixedCommission);
    const feeBps = parseNumber(options["fee-bps"], config.backtest.feeBps);
